...
function etp_check_elementor_dependency() {
	if ( ! did_action( 'elementor/loaded' ) ) {
		add_action( 'admin_notices', 'etp_elementor_missing_notice' );
		return false;
	}
	return true;
}
...
function etp_elementor_missing_notice() {
	$message = sprintf(
...
class Elementor_Template_Porter {
	...
	public function __construct() {
		...
		// AJAX handlers
		add_action( 'wp_ajax_etp_export_template', [ $this, 'ajax_export_template' ] );
		add_action( 'wp_ajax_etp_import_template', [ $this, 'ajax_import_template' ] );
	...
	public function enqueue_scripts( $hook ) {
		...
		wp_enqueue_style( 'etp-admin', ETP_PLUGIN_URL . 'assets/admin.css', [], ETP_VERSION );
		wp_enqueue_script( 'etp-admin', ETP_PLUGIN_URL . 'assets/admin.js', [ 'jquery' ], ETP_VERSION, true );
		wp_localize_script( 'etp-admin', 'etpData', [
			'ajax_url' => admin_url( 'admin-ajax.php' ),
			'nonce'    => wp_create_nonce( 'etp_nonce' ),
		] );
	}
	...
	public function admin_page() {
		...
		include ETP_PLUGIN_DIR . 'views/admin-page.php';
	}

	// ========== EXPORT ==========
	public function ajax_export_template() {
		check_ajax_referer( 'etp_nonce', 'nonce' );
	...
	protected function export_template_with_images( $template_id ) {
		...
		$temp_dir   = trailingslashit( $upload_dir['basedir'] ) . 'etp-temp-' . time();
		wp_mkdir_p( $temp_dir );
	...
	protected function import_template_with_images( $file ) {
		...
		$upload_dir = wp_upload_dir();
		$temp_dir   = trailingslashit( $upload_dir['basedir'] ) . 'etp-import-' . wp_generate_password( 12, false );
		wp_mkdir_p( $temp_dir );
...
function etp_init() {
	if ( etp_check_elementor_dependency() ) {
		new Elementor_Template_Porter();
	}
}
add_action( 'plugins_loaded', 'etp_init' );